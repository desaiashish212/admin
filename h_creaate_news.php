		...
		if(!file_exists($date))
		{
			 ...
			 foreach($_FILES['files']['tmp_name'] as $key => $tmp_name )
			 {
				...
				$file_type=$_FILES['files']['type'][$key];	
					...
					if($count === 0)
					{
						$res=mysqli_query($con,"insert into hindi (title,news,time,status,path,date) values ('$title','$news','$server_time','$status','$path','$newsDate')");
						
						$count++; 	// on next iteration execute else
						
					}else{
						$res=mysqli_query($con,"update hindi SET path2='$path' WHERE hindi.title='$title'");					
					}
					
					   	$_SESSION["MSG"]="News not added, Try Again";
						header("Location: h_news.php?id=".$status."&name=".$category);	
