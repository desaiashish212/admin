			...
			<div class="container-fluid">
				...
				while($row=mysqli_fetch_array($sql))
				{
					...
					?>
						...
						<td width="50"><a href="m_edit_news.php?m_id=<?= $id ;?>&id=<?=$news_id;?>&category=<?=$_GET["name"];?>" class="btn btn-primary">Edit</a></td>
						<td width="50"><a href="m_delete_news.php?m_id=<?= $id ;?>&id=<?=$news_id;?>&category=<?=$_GET["name"];?>" class="btn btn-danger">Delete</a></td>
						<?
							if($row['5']==0){
						?>
						<td><a href="worker_news_notifications.php?m_id=<?= $id ;?>&id=<?= $news_id ;?>&lang=<?='1';?>&title=<?=$row['1'];?>&category=<?=$_GET["name"];?>&status=<?=$_GET['id'];?>" class="btn btn-info">Notify</a></td>
						<?
							}else{
						?>
