				...
				while($row=mysqli_fetch_array($sql))
				{
					...
					?>
						...
						<?
							if($row['5']==0){
						?>
						<td><a href="worker_news_notifications.php?m_id=<?= $id ;?>&id=<?= $news_id ;?>&lang=<?='1';?>&title=<?=$row['1'];?>&category=<?=$_GET["name"];?>&status=<?=$_GET['id'];?>" class="btn btn-primary btn-sm">Notify</a></td>
						<?
							}else{
						?>
